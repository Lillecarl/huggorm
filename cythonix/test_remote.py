...
def check_no_hardcoded_domain_types() -> None:
    ...
    here = pathlib.Path(__file__).parent / "cythonix"
    offenders = []
...
async def main() -> None:
    ...
    server = await asyncio.create_subprocess_exec(
        sys.executable, "-m", "cythonix.server", HOST, str(PORT),
        stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
    logs: list[str] = []
    ...
    try:
        ...
        check("wire-value return is a real local object",
              type(p).__module__ == "cythonix_bindings.store", type(p).__name__)
        ...
        from cythonix_generated import RPC_CLASSES, RPCDerivation, RPCValue
        ...
        check("generated client class, not a __getattr__ proxy",
              ...
              and type(drv).__module__ == "cythonix_generated.rpc",
              type(drv).__module__)
        ...
        check("the map agrees with the in-process call",
              set(stats) == set(cythonix_bindings.gc_stats()), sorted(stats))
        threw = None
        try:
