...
`src/huggorm_idl/decl/`. It is never RUN. Everything in
`huggorm_bindings` and everything the generator writes above it comes
...
type is a HANDLE - `huggorm::Bridge` roots a GC-resident value and
...
owns. Both are counted, and the build prints the ratio per class:
    ...
    decl/path.py -> .../path.cpp (module huggorm_bindings.path): StorePath
